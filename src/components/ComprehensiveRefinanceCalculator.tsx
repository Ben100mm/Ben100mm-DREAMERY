...
const ComprehensiveRefinanceCalculator: React.FC<{ onResultsChange?: (results: any) => void }> = ({ onResultsChange }) => {
  ...
  const [activeScenario, setActiveScenario] = useState<'rate-and-term' | 'cash-out' | 'cash-in'>('rate-and-term');
  ...
  const scenarios = useMemo((): RefinanceScenario[] => {
    ...
    const rateTermPayment = calculateMonthlyPayment(rateTermLoan, inputs.newInterestRate, inputs.newTerm * 12);
    ...
    const cashOutPayment = calculateMonthlyPayment(cashOutLoan, inputs.newInterestRate, inputs.newTerm * 12);
    const cashOutSavings = inputs.currentMonthlyPayment - cashOutPayment;
    const cashOutBreakEven = closingCosts / cashOutSavings;

    // Cash-in refinance
    const cashInLoan = Math.max(0, inputs.currentLoanBalance - inputs.cashInAmount);
    const cashInPayment = calculateMonthlyPayment(cashInLoan, inputs.newInterestRate, inputs.newTerm * 12);
    ...
    return [
      {
        ...
        pros: ['Lower monthly payment', 'Lower total interest paid', 'No cash required'],
      ...
      },
      {
        ...
        description: 'Access your property equity for other investments or expenses',
        pros: ['Access to equity', 'Lower rate possible', 'Tax-deductible interest'],
        cons: ['Higher loan balance', 'Higher monthly payment', 'Closing costs'],
      ...
      },
      {
        ...
        description: 'Put money into your property to lower LTV and get better rates',
  ...
  const npv = useMemo(() => {
    const closingCosts = inputs.currentLoanBalance * (inputs.closingCostsPercent / 100);
    const monthlySavings = scenarios.find(s => s.type === activeScenario)?.monthlySavings || 0;
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <Card sx={{ borderRadius: 2, border: '1px solid #e0e0e0' }}>
        <CardContent>
          <Typography variant="h6" sx={{ fontWeight: 600, color: '#1a365d', mb: 3 }}>
          ...
          <Box sx={{ display: 'grid', gap: 3, gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' } }}>
            ...
            <Box>
              <TextField
                ...
                onChange={(e) => handleInputChange('closingCostsPercent', parseFloat(e.target.value) || 0)}
                InputProps={{
                  endAdornment: <InputAdornment position="end">%</InputAdornment>,
                }}
          ...
          <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, mb: 3 }}>
            {scenarios.map((scenario) => (
              <Box key={scenario.type}>
                <Button
                  variant={activeScenario === scenario.type ? 'contained' : 'outlined'}
                  fullWidth
                  ...
                  sx={{
                    ...
                    gap: 1,
                    bgcolor: activeScenario === scenario.type ? '#1a365d' : 'transparent',
                    color: activeScenario === scenario.type ? 'white' : '#1a365d',
                    '&:hover': {
                      bgcolor: activeScenario === scenario.type ? '#1a365d' : '#f5f5f5',
                    },
                  }}
                >
          ...
          {selectedScenario && (
            <Box>
              <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap' }}>
                    ...
                    color={selectedScenario.monthlySavings > 0 ? 'success' : 'error'}
                  />
                <Chip
                  icon={<TimelineIcon />}
                  label={`Break-even: ${selectedScenario.breakEvenMonths.toFixed(1)} months`}
                  color={selectedScenario.breakEvenMonths < 24 ? 'success' : 'warning'}
                />
                <Chip
                  icon={<AccountBalanceIcon />}
                  label={`New LTV: ${selectedScenario.ltv.toFixed(1)}%`}
                  color={selectedScenario.ltv < 70 ? 'success' : selectedScenario.ltv < 80 ? 'warning' : 'error'}
                />
              </Box>
            ...
            <CardContent>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Typography variant="h6" sx={{ fontWeight: 600, color: npv > 0 ? '#2e7d32' : '#d32f2f' }}>
                    {formatCurrency(npv.toString())}
                  </Typography>
                </Box>
                
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Typography variant="body2">Total Savings (30 years):</Typography>
                  <Typography variant="h6" sx={{ fontWeight: 600, color: '#2e7d32' }}>
                    {formatCurrency((selectedScenario?.totalSavings || 0).toString())}
                  </Typography>
                </Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Typography variant="h6" sx={{ fontWeight: 600, color: '#d32f2f' }}>
                    {formatCurrency((inputs.currentLoanBalance * (inputs.closingCostsPercent / 100)).toString())}
                  </Typography>
                </Box>
              </Box>
            </CardContent>
            ...
            <CardContent>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Typography variant="h6" sx={{ fontWeight: 600, color: (inputs.currentLoanBalance / inputs.propertyValue) * 100 > 80 ? '#d32f2f' : '#2e7d32' }}>
                    {((inputs.currentLoanBalance / inputs.propertyValue) * 100).toFixed(1)}%
                  </Typography>
                </Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Chip
                    label={inputs.creditScore >= 800 ? 'Excellent' : inputs.creditScore >= 740 ? 'Good' : inputs.creditScore >= 670 ? 'Fair' : 'Poor'}
                    color={inputs.creditScore >= 740 ? 'success' : inputs.creditScore >= 670 ? 'warning' : 'error'}
                    size="small"
                  />
                </Box>
              </Box>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                {riskScore > 6 && (
                  <Alert severity="error" sx={{ mb: 1 }}>
                    High risk - focus on improving financial position before refinancing
                  </Alert>
                )}
