...
import { Box, Card, Typography, TextField, Button, Alert, InputAdornment, CircularProgress } from '@mui/material';
import { DealState } from '../types/deal';
import { defaultMarketConditionsSimple, defaultRiskFactors, defaultExitStrategies } from '../utils/advancedCalculations';
      ...
      <Card sx={{ p: 2, backgroundColor: '#f8f9fa' }}>
        ...
        <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: '1fr 1fr 1fr' } }}>
          <TextField
            ...
            value={((dealState.marketConditions?.inflationRate || defaultMarketConditionsSimple.inflationRate) * 100).toFixed(1)}
            onChange={(e) => {
              const rawValue = e.target.value;
              if (rawValue === '') return; // Allow empty for reset
              const value = Math.max(0, parseFloat(rawValue) || 0) / 100;
              updateDealState({
                marketConditions: { ...(dealState?.marketConditions || defaultMarketConditionsSimple), inflationRate: value }
              });
            }}
            InputProps={{
              endAdornment: <InputAdornment position="end">%</InputAdornment>,
            }}
            helperText={parseFloat(((dealState.marketConditions?.inflationRate || defaultMarketConditionsSimple.inflationRate) * 100).toFixed(1)) < 0 ? 'Must be 0 or greater' : ''}
          ...
          <TextField
            ...
            value={((dealState.marketConditions?.appreciationRate || defaultMarketConditionsSimple.appreciationRate) * 100).toFixed(1)}
            onChange={(e) => {
              const rawValue = e.target.value;
              if (rawValue === '') return; // Allow empty for reset
              const value = Math.max(0, parseFloat(rawValue) || 0) / 100;
              updateDealState({
                marketConditions: { ...(dealState?.marketConditions || defaultMarketConditionsSimple), appreciationRate: value }
              });
            }}
            InputProps={{
              endAdornment: <InputAdornment position="end">%</InputAdornment>,
            }}
            helperText={parseFloat(((dealState.marketConditions?.appreciationRate || defaultMarketConditionsSimple.appreciationRate) * 100).toFixed(1)) < 0 ? 'Must be 0 or greater' : ''}
          ...
          <TextField
            ...
            value={((dealState.marketConditions?.rentGrowthRate || defaultMarketConditionsSimple.rentGrowthRate) * 100).toFixed(1)}
            onChange={(e) => {
              const rawValue = e.target.value;
              if (rawValue === '') return; // Allow empty for reset
              const value = Math.max(0, parseFloat(rawValue) || 0) / 100;
              updateDealState({
                marketConditions: { ...(dealState?.marketConditions || defaultMarketConditionsSimple), rentGrowthRate: value }
              });
            }}
            InputProps={{
              endAdornment: <InputAdornment position="end">%</InputAdornment>,
            }}
            helperText={parseFloat(((dealState.marketConditions?.rentGrowthRate || defaultMarketConditionsSimple.rentGrowthRate) * 100).toFixed(1)) < 0 ? 'Must be 0 or greater' : ''}
          ...
          <Button 
            ...
            onClick={() => {
              if (Object.keys(allResults).length === 0) {
                setSnackbar({ 
                  open: true, 
                  message: 'No results to save. Please complete some calculations first.', 
                  severity: 'warning' 
                });
                return;
              }
