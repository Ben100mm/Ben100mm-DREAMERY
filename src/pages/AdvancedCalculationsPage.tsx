...
import { LineChart, LineSeriesType, BarChart, BarSeriesType, PieChart, PieSeriesType } from '@mui/x-charts';
import { useAuth } from '../contexts/AuthContext';
import { saveDealToCloud, loadDealsFromCloud, deleteDealFromCloud } from '../firebase/services';
...
const AdvancedCalculationsPage: React.FC = () => {
  ...
  const tourSteps = [
    {
      ...
      content: 'Welcome to Advanced Analysis. Begin by completing the Global Configuration to unlock comprehensive financial modeling capabilities.',
      placement: 'bottom',
    },
    {
      target: 'guided-tour-button',
      content: 'Access comprehensive guidance and support through our interactive tour system.',
      placement: 'bottom',
    },
    {
      target: 'completion-progress',
      content: 'Monitor your analysis completion status and track progress across all modules.',
      placement: 'top',
    },
    {
      target: 'stepper-navigation',
      content: 'Navigate seamlessly between analysis modules using our intuitive step-by-step interface.',
      placement: 'bottom',
    },
    {
      target: 'floating-config-button',
      content: 'Efficiently access configuration settings from any analysis module.',
      placement: 'left',
    },
    {
      target: 'undo-button',
      content: 'Maintain data integrity with our comprehensive undo and reset functionality.',
      placement: 'top',
    },
  ];
  ...
  useEffect(() => {
    if (dealState) {
      const results = calculateExitStrategies(dealState?.purchasePrice || 300000, dealState?.exitStrategies || defaultExitStrategies, dealState?.purchasePrice || 300000);
  ...
  useEffect(() => {
    if (dealState) {
      ...
      let annualIncome = 0;
      if (dealState.revenueInputs && dealState.revenueInputs.totalRooms > 0 && dealState.revenueInputs.averageDailyRate > 0) {
        // Use actual revenue inputs for hotels/STR
        annualIncome = dealState.revenueInputs.totalRooms * dealState.revenueInputs.averageDailyRate * (dealState.revenueInputs.occupancyRate / 100) * 365;
      } else {
      ...
      const results = calculateTaxImplications(annualIncome, propertyExpenses, dealState.taxImplications);
      updateDealState({ taxImplicationResults: results });
      handleResultsChange('tax', results);
    }
  }, [dealState?.taxImplications, dealState?.purchasePrice, dealState?.revenueInputs, dealState?.loan?.monthlyPayment, dealState?.proForma?.taxes, dealState?.proForma?.maintenance]);
  ...
  const checkConfigurationStatus = (state: DealState | null) => {
    ...
    const hasMarketConditions = !!state.marketConditions && 
      Object.keys(state.marketConditions).length > 0;
    const hasRiskFactors = !!state.riskFactors && 
      Object.keys(state.riskFactors).length > 0;
    const hasExitStrategies = !!state.exitStrategies && 
      state.exitStrategies.length > 0;
    const hasTaxImplications = !!state.taxImplications && 
      Object.keys(state.taxImplications).length > 0;
    
    const isComplete = hasMarketConditions && hasRiskFactors && hasExitStrategies && hasTaxImplications;
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && event.key === '3' && configsComplete) {
        event.preventDefault();
        setTabValue(2);
      }
    };
  ...
  useEffect(() => {
    if (Object.keys(allResults).length > 0) {
      try {
        ...
        console.log('Calculation results automatically saved to sessionStorage');
      } catch (error) {
    ...
    if (dealState) {
      const fetchMarketData = async () => {
        try {
          ...
          updateDealState({
            marketDataLoading: false,
            marketDataError: 'Failed to fetch live market data. Using default values.',
  ...
  const handleTabChange = (event: React.SyntheticEvent, newValue: number) => {
    console.log('Tab change requested:', { currentTab: tabValue, newTab: newValue });
  ...
  const handleResultsChange = <K extends keyof CalculatorResults>(
    ...
    const calculatorName = calculatorNames[calculatorType as string] || calculatorType;
    setSnackbar({ 
      open: true, 
  ...
  const handleBackToUnderwrite = () => {
    ...
    try {
      ...
      console.error('React Router navigation failed, trying direct navigation:', error);
    ...
    if (isComplete && !configsComplete) {
      ...
      setTabValue(2); // Switch to first unlocked tab (Exit Strategies)
        ...
        setTimeout(() => {
          ...
          const header = document.querySelector('[data-testid="config-status-alert"]') as HTMLElement | null;
          if (header) {
          ...
          setTimeout(() => {
            const successAlert = document.querySelector('[data-testid="config-status-alert"]') as HTMLElement | null;
            if (successAlert) {
  ...
  const handleSaveToCloud = async () => {
    ...
    try {
      const result = await saveDealToCloud(dealState, currentDealId || undefined);
      if (result.success) {
  ...
  const handleDeleteFromCloud = async (dealId: string) => {
    ...
    if (window.confirm('Are you sure you want to delete this deal from the cloud?')) {
      setCloudLoading(true);
      try {
  ...
  console.log('Tab config created:', tabConfig.length, 'tabs, current tabValue:', tabValue);
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: '#ffffff', transition: 'all 0.3s ease-in-out' }}>
                ...
                <Box sx={{ 
                  ...
                  <Typography variant="caption" sx={{ 
                    ...
                    {(() => {
                      ...
                      return configsComplete ? 'Ready' : `${completed}/4 Config`;
                    })()}
                  </Typography>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              ...
              {dealState?.lastMarketDataUpdate && (
                <Button
                  ...
                  startIcon={<span>-</span>}
                >
                  Market Data: {new Date(dealState.lastMarketDataUpdate).toLocaleString()}
                </Button>
              )}
            ...
            <Box sx={{ 
              ...
              {dealHistory.length > 1 && (
                <Box sx={{ 
                  ...
                  <Typography variant="body2" sx={{ color: '#1a365d', fontWeight: 500 }}>
                    History: {dealHistory.length - 1} step{dealHistory.length > 2 ? 's' : ''} available
                  </Typography>
                </Box>
              )}
              ...
              {/* Undo/Reset Controls */}
                <Button
                  ...
                  aria-label={canUndo ? `Undo last change (${dealHistory.length - 1} steps available)` : 'No changes to undo'}
                  data-testid="undo-button"
                  sx={{
                    borderColor: canUndo ? '#1a365d' : '#ccc',
                    color: canUndo ? '#1a365d' : '#ccc',
                    '&:hover': canUndo ? { borderColor: '#0f2027', bgcolor: '#f0f8ff' } : {},
                    minWidth: 80,
                    px: 2,
                    py: 0.75,
                    minHeight: 36
                ...
                <Button
                  ...
                  aria-label={canReset ? 'Reset to initial state' : 'No initial state to reset to'}
                  data-testid="reset-button"
                  sx={{
                    borderColor: canReset ? '#dc3545' : '#ccc',
                    color: canReset ? '#dc3545' : '#ccc',
                    '&:hover': canReset ? { borderColor: '#c82333', bgcolor: '#fff5f5' } : {},
                    minWidth: 80,
                    px: 2,
                    py: 0.75,
                    minHeight: 36
      ...
      {dealState ? (
        <Box>
          <Alert severity="success" sx={{ mb: 2, bgcolor: '#f0f8ff', borderColor: '#1a365d' }} data-testid="deal-loaded-alert">
            <Typography variant="body2" sx={{ color: '#1a365d' }}>
              <strong>Deal Loaded:</strong> {dealState.propertyType} property in {dealState.city}, {dealState.state} - ${dealState.purchasePrice?.toLocaleString()}
            </Typography>
            <Typography variant="caption" sx={{ color: '#1a365d', opacity: 0.8, display: 'block', mt: 0.5 }}>
          ...
          {!configsComplete && (
            <Alert severity="info" sx={{ mb: 2, bgcolor: '#e3f2fd', borderColor: '#1976d2' }} data-testid="config-status-alert">
              <Typography variant="body2" sx={{ color: '#1976d2', mb: 1 }}>
                <strong>Configuration Required:</strong> Complete the Global Configuration tab to unlock advanced analysis features.
                <Button
                  size="small"
                  variant="text"
                  onClick={() => setTabValue(1)}
              ...
              <Typography variant="caption" sx={{ color: '#1976d2', opacity: 0.8, display: 'block', mb: 1 }}>
                Use Ctrl/Cmd + 2 to quickly jump to the Global Configuration tab.
              </Typography>
                  ...
                  return (
                    <>
                      <Chip 
                        ...
                        size="small" 
                        color={status.hasMarketConditions ? "success" : "default"}
                        variant={status.hasMarketConditions ? "filled" : "outlined"}
                      />
                      <Chip 
                        label="Risk Factors" 
                        size="small" 
                        color={status.hasRiskFactors ? "success" : "default"}
                        variant={status.hasRiskFactors ? "filled" : "outlined"}
                      />
                      <Chip 
                        label="Exit Strategies" 
                        size="small" 
                        color={status.hasExitStrategies ? "success" : "default"}
                        variant={status.hasExitStrategies ? "filled" : "outlined"}
                      />
                      <Chip 
                        label="Tax Implications" 
                        size="small" 
                        color={status.hasTaxImplications ? "success" : "default"}
                        variant={status.hasTaxImplications ? "filled" : "outlined"}
                      />
                    </>
                  );
                })()}
            ...
            {showConfigSuccess && (
              ...
              >
                <Typography variant="body2" sx={{ color: '#2e7d32' }}>
                  <strong>Configuration Complete!</strong> All advanced analysis features are now unlocked and ready to use.
                </Typography>
              </Alert>
            )}
              ...
              <Box sx={{ 
                ...
                <Box sx={{ flex: 1, width: '100%' }}>
                  <Typography variant="body2" sx={{ 
                    ...
                    <strong>Configuration Progress:</strong> Complete these steps to unlock advanced features
                  </Typography>
                  <Box sx={{ 
                    display: 'grid', 
                    gap: { xs: 1, sm: 1.5 }, 
                      ...
                      return (
                        <>
                          <Chip 
                            ...
                            color={status.hasMarketConditions ? "success" : "default"}
                            variant={status.hasMarketConditions ? "filled" : "outlined"}
                            icon={status.hasMarketConditions ? <span>Complete</span> : <span>LOCK</span>}
                          ...
                          <Chip 
                            ...
                            color={status.hasRiskFactors ? "success" : "default"}
                            variant={status.hasRiskFactors ? "filled" : "outlined"}
                            icon={status.hasRiskFactors ? <span>Complete</span> : <span>LOCK</span>}
                            sx={{ fontSize: { xs: '0.7rem', sm: '0.75rem' } }}
                          />
                          <Chip 
                            label="Exit" 
                            ...
                            color={status.hasExitStrategies ? "success" : "default"}
                            variant={status.hasExitStrategies ? "filled" : "outlined"}
                            icon={status.hasExitStrategies ? <span>Complete</span> : <span>LOCK</span>}
                            sx={{ fontSize: { xs: '0.7rem', sm: '0.75rem' } }}
                          />
                          <Chip 
                            label="Tax" 
                            ...
                            color={status.hasTaxImplications ? "success" : "default"}
                            variant={status.hasTaxImplications ? "filled" : "outlined"}
                            icon={status.hasTaxImplications ? <span>Complete</span> : <span>LOCK</span>}
                            sx={{ fontSize: { xs: '0.7rem', sm: '0.75rem' } }}
                          />
                        </>
                      );
            ...
            <Card sx={{ borderRadius: 2, border: '1px solid #e0e0e0', transition: 'all 0.3s ease-in-out', minHeight: 'fit-content' }}>
              <CardContent sx={{ p: 3 }}>
                ...
                {/* Configuration Guide */}
                {!configsComplete && (
                  <Alert severity="info" sx={{ mb: 3, bgcolor: '#e3f2fd', borderColor: '#1976d2' }}>
                    <Typography variant="body2" sx={{ color: '#1976d2', mb: 1 }}>
                      <strong>Getting Started:</strong> Complete the Global Configuration tab to unlock advanced analysis features.
                    </Typography>
                    <Typography variant="caption" sx={{ color: '#1976d2', opacity: 0.8, display: 'block' }}>
                      - Configure Market Conditions (inflation, appreciation, rent growth rates)
                    </Typography>
                    <Typography variant="caption" sx={{ color: '#1976d2', opacity: 0.8, display: 'block' }}>
                      - Set Risk Factors (market volatility, tenant quality, property age)
                    </Typography>
                    <Typography variant="caption" sx={{ color: '#1976d2', opacity: 0.8, display: 'block' }}>
                      - Define Exit Strategies (timeline, selling costs, ROI targets)
                    </Typography>
                    <Typography variant="caption" sx={{ color: '#1976d2', opacity: 0.8, display: 'block' }}>
                      - Configure Tax Implications (bracket, deductions, property expenses)
                    </Typography>
                  </Alert>
                )}
                ...
                <Box sx={{ mt: 3, p: 2, bgcolor: '#f8f9fa', borderRadius: 1, border: '1px solid #e0e0e0' }}>
                      ...
                      return (
                        <>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <Chip label={status.hasMarketConditions ? "Complete" : "Required"} size="small" color={status.hasMarketConditions ? "success" : "default"} variant={status.hasMarketConditions ? "filled" : "outlined"} />
                          </Box>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Typography variant="body2" sx={{ color: '#666' }}>
                              Risk Factors
                            </Typography>
                            <Chip label={status.hasRiskFactors ? "Complete" : "Required"} size="small" color={status.hasRiskFactors ? "success" : "default"} variant={status.hasRiskFactors ? "filled" : "outlined"} />
                          </Box>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Typography variant="body2" sx={{ color: '#666' }}>
                              Exit Strategies
                            </Typography>
                            <Chip label={status.hasExitStrategies ? "Complete" : "Required"} size="small" color={status.hasExitStrategies ? "success" : "default"} variant={status.hasExitStrategies ? "filled" : "outlined"} />
                          </Box>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Typography variant="body2" sx={{ color: '#666' }}>
                              Tax Implications
                            </Typography>
                            <Chip label={status.hasTaxImplications ? "Complete" : "Required"} size="small" color={status.hasTaxImplications ? "success" : "default"} variant={status.hasTaxImplications ? "filled" : "outlined"} />
                          </Box>
                        </>
                      );
                    })()}
                  ...
                  {configsComplete && (
                    ...
                    >
                      <Typography variant="body2" sx={{ color: '#2e7d32' }}>
                        All configurations complete! Advanced analysis features are now unlocked.
                      </Typography>
                      <Typography variant="caption" sx={{ color: '#2e7d32', opacity: 0.8, display: 'block', mt: 0.5 }}>
                        You can now access all tabs and advanced analysis features.
                      </Typography>
                      <Typography variant="caption" sx={{ color: '#2e7d32', opacity: 0.8, display: 'block', mt: 0.5 }}>
                        Pro tip: Use Ctrl/Cmd + 3 to jump to the first unlocked tab.
                      </Typography>
                    </Alert>
                  )}
                </Box>
            ...
            <Card sx={{ mt: 2, borderRadius: 2, border: '1px solid #e0e0e0' }}>
              <CardContent sx={{ p: 3 }}>
                <Alert severity="info" sx={{ bgcolor: '#e3f2fd', borderColor: '#1976d2' }}>
                  <Typography variant="body2" sx={{ color: '#1976d2' }}>
                    <strong>Enhanced Capabilities:</strong> Our comprehensive analysis suite provides sophisticated financial modeling and risk assessment tools. 
                    Leverage advanced algorithms for deeper insights into your real estate investment strategies.
                  </Typography>
                </Alert>
              </CardContent>
            </Card>
            ...
            <Card sx={{ mt: 2, borderRadius: 2, border: '1px solid #e0e0e0' }}>
              ...
              <CardContent sx={{ p: 0 }}>
                  ...
                  <Box sx={{ 
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <Typography variant="body2" sx={{ 
                        ...
                        Complete Global Configuration to unlock comprehensive analysis capabilities
                      </Typography>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <Typography variant="body2" sx={{ 
                ...
                <Box sx={{ borderBottom: 1, borderColor: '#e0e0e0', bgcolor: '#fafbfc' }}>
                  {/* ============================================================================
                     TABS STYLING
                     ============================================================================ */}
                  ...
                  <Tabs
                    ...
                    variant="scrollable"
                    scrollButtons="auto"
                    onClick={() => console.log('Tabs clicked, current tabValue:', tabValue)}
                    sx={{
                      '& .MuiTab-root': {
                        color: '#666',
                        fontWeight: 600,
                  ...
                  >
                    {/* ============================================================================
                      ...
                      <Tab
                        ...
                        sx={{}}
                        onClick={() => console.log(`Tab ${index} (${tab.label}) clicked directly`)}
                      />
                    ))}
                  </Tabs>
                </Box>
            ...
            {/* ============================================================================
              ...
              <FeaturesOverview />
                ...
                <Box sx={{ display: 'grid', gap: 3, gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' } }}>
                  <Box>
                    <Accordion sx={{ '& .MuiAccordionSummary-root': { bgcolor: '#fafbfc' }, '& .MuiAccordionDetails-root': { bgcolor: '#ffffff' } }}>
                      <AccordionSummary expandIcon={<ExpandMoreIcon sx={{ color: '#1a365d' }} />}>
                        <Typography variant="h6" sx={{ fontWeight: 600, color: '#1a365d' }}>Advanced Scenarios</Typography>
                      </AccordionSummary>
                      <AccordionDetails>
                    ...
                    <Accordion sx={{ '& .MuiAccordionSummary-root': { bgcolor: '#fafbfc' }, '& .MuiAccordionDetails-root': { bgcolor: '#ffffff' } }}>
                      <AccordionSummary expandIcon={<ExpandMoreIcon sx={{ color: '#1a365d' }} />}>
                        <Typography variant="h6" sx={{ color: '#1a365d', fontWeight: 600 }}>Risk Analysis</Typography>
                      </AccordionSummary>
                      <AccordionDetails>
                          ...
                          {dealState?.riskScoreResults && (
                            <>
                              <Chip 
                                ...
                                color={
                                  dealState.riskScoreResults.overallRiskScore <= 3 ? 'success' :
                                  dealState.riskScoreResults.overallRiskScore <= 5 ? 'warning' : 'error'
                                }
                                variant="filled"
                                sx={{ 
                                  color: dealState.riskScoreResults.overallRiskScore > 5 ? '#d32f2f' : '#2e7d32',
                                  fontWeight: 700,
                                  fontSize: '1rem'
                                }}
                              />
            ...
            <Card sx={{ mt: 2, borderRadius: 2, border: '1px solid #e0e0e0' }}>
              <CardContent sx={{ p: 3 }}>
                ...
                <Typography variant="body1" sx={{ mb: 3, color: '#666' }}>
                  Our sophisticated analysis tools seamlessly integrate with your existing mortgage and underwriting workflows 
                  to provide more sophisticated analysis without breaking current functionality.
                </Typography>
                
                <Box sx={{ display: 'grid', gap: 3, gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' } }}>
                  <Box>
                    <Box sx={{ p: 2, bgcolor: '#fafbfc', borderRadius: 1, border: '1px solid #e8eaed' }}>
                      <Typography variant="h6" gutterBottom sx={{ fontWeight: 600, color: '#1a365d' }}>
                        For Mortgage Calculations
                      </Typography>
                      <Typography variant="body2" sx={{ mb: 1, color: '#666' }}>
                        - Use seasonal adjustments to modify vacancy rates in your DSCR calculations
                      </Typography>
                      <Typography variant="body2" sx={{ mb: 1, color: '#666' }}>
                        - Apply market conditions to adjust rent growth and appreciation rates
                      </Typography>
                      <Typography variant="body2" sx={{ mb: 1, color: '#666' }}>
                        - Include tax implications to show net income after deductions
                      </Typography>
                    </Box>
                  </Box>
                  ...
                  <Box>
                    <Box sx={{ p: 2, bgcolor: '#fafbfc', borderRadius: 1, border: '1px solid #e8eaed' }}>
                      ...
                      <Typography variant="body2" paragraph sx={{ color: '#666' }}>
                        - Include exit strategies to show different ROI timelines
                      </Typography>
                    </Box>
                  </Box>
                </Box>
          ...
          <Box>
            <Alert severity="warning" sx={{ mb: 2 }}>
              <Typography variant="body2">
                <strong>No Deal Data:</strong> Please go back to the Underwrite page and click "Open Advanced Analysis" to load your deal data. Advanced features are disabled until a deal is loaded.
              </Typography>
            </Alert>
          </Box>
        )}
