          ...
          <QuestionCard>
            <CardContent sx={{ p: { xs: 1.5, md: 3 }, flex: 1, overflow: 'auto' }}>
              ...
              <Typography variant="body2" sx={{ color: '#666', mb: 4, textAlign: 'center' }}>
                You'll get connected with a dedicated loan officer to verify you're on track, explore your loan options and upgrade to a Verified Pre-approval.
              </Typography>
                ...
                <Box sx={{ display: 'grid', gap: 1.5 }}>
                  {answers.firstName && answers.lastName && (
                    <Typography variant="body2" sx={{ color: '#666' }}>
                      <strong>Name:</strong> {answers.firstName} {answers.lastName}
                    </Typography>
                  )}
                  ...
                  {answers.streetAddress && answers.city && answers.state && (
                    <Typography variant="body2" sx={{ color: '#666' }}>
                      <strong>Current Address:</strong> {answers.streetAddress}, {answers.city}, {answers.state} {answers.zipCode}
                    </Typography>
                  )}
                  {answers.income && (
                    <Typography variant="body2" sx={{ color: '#666' }}>
                      <strong>Annual Income:</strong> ${(() => {
                        const num = parseInt(answers.income.replace(/[^0-9]/g, ''));
                  ...
                  {answers.homePrice && (
                    <Typography variant="body2" sx={{ color: '#666' }}>
                      <strong>Target Home Price:</strong> ${(() => {
                        const num = parseInt(answers.homePrice.replace(/[^0-9]/g, ''));
                        return isNaN(num) ? '0' : num.toLocaleString();
                      })()}
                    </Typography>
                  )}
                  {answers.monthlyPayment && (
                    <Typography variant="body2" sx={{ color: '#666' }}>
                      <strong>Target Monthly Payment:</strong> ${(() => {
                        const num = parseInt(answers.monthlyPayment.replace(/[^0-9]/g, ''));
                        return isNaN(num) ? '0' : num.toLocaleString();
                      })()}/month
                    </Typography>
                  )}
              ...
              {/* Next Steps */}
              <Box sx={{ backgroundColor: '#f5f5f5', p: 2, borderRadius: 2, mb: 4 }}>
