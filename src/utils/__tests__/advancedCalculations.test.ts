...
describe("advancedCalculations", () => {
  test("calculateSeasonalAdjustments returns adjusted vacancy and maintenance multiplier", () => {
    ...
    const res = calculateSeasonalAdjustments(
      baseVacancy,
      defaultSeasonalFactors,
      7,
    ); // July -> summerVacancyRate 0.1
    expect(res.adjustedVacancyRate).toBeCloseTo(
      baseVacancy * (1 + defaultSeasonalFactors.summerVacancyRate),
    );
    expect(res.maintenanceMultiplier).toBe(
      defaultSeasonalFactors.seasonalMaintenanceMultiplier,
    );
  });
  ...
  test("calculateMarketAdjustments applies market conditions", () => {
    ...
    expect(res.adjustedVacancyRate).toBeCloseTo(
      base.vacancyRate * (1 + hot.vacancyRateAdjustment),
    );
    expect(res.adjustedRentGrowth).toBeCloseTo(
      base.rentGrowth * (1 + hot.rentGrowthRate),
    );
    expect(res.adjustedAppreciation).toBeCloseTo(
      base.appreciation * (1 + hot.appreciationRate),
    );
    expect(res.adjustedCapRate).toBeCloseTo(
      base.capRate * (1 + hot.capRateAdjustment),
    );
  });
  ...
  test("calculateExitStrategies computes ROI and annualized ROI", () => {
    ...
    const res = calculateExitStrategies(
      propertyValue,
      strategies,
      currentMarketValue,
    );
  ...
  test("calculateRefinanceScenarios returns meaningful savings and break-even", () => {
    ...
    const res = calculateRefinanceScenarios(
      currentLoan,
      scenarios,
      propertyValue,
    );
