...
import { pmt, remainingPrincipalAfterPayments, totalMonthlyDebtService, breakEvenOccupancy, brrrrAnnualCashFlowPostRefi } from '../finance';
...
describe('finance utils', () => {
  ...
  it('totalMonthlyDebtService sums all parts', () => {
    ...
    expect(totalMonthlyDebtService({ newLoanMonthly: 1000, subjectToMonthlyTotal: 300 })).toBe(1300);
    expect(totalMonthlyDebtService({ newLoanMonthly: 1000, subjectToMonthlyTotal: 300, hybridMonthly: 200 })).toBe(1500);
  });
  ...
  it('breakEvenOccupancy respects variable toggle', () => {
    ...
    const fixed = 2000;
    const varPct = 0.1; // 10%
    const off = breakEvenOccupancy({ monthlyRevenueAt100, fixedMonthlyOps: fixed, variablePct: varPct, includeVariablePct: false });
    const on = breakEvenOccupancy({ monthlyRevenueAt100, fixedMonthlyOps: fixed, variablePct: varPct, includeVariablePct: true });
  ...
  it('brrrrAnnualCashFlowPostRefi basic scenario', () => {
    const cf = brrrrAnnualCashFlowPostRefi({ monthlyRevenue: 6000, fixedMonthlyOps: 2000, variablePct: 0.1, newLoanMonthly: 1500 });
